...
// Assuming Badge is still used elsewhere, otherwise it could be removed
...
export default function MisIncidenciasPage() {
  ...
  const [lastUpdate, setLastUpdate] = useState(new Date()); // lastUpdate state is kept, even if not displayed, as it's updated by loadIncidents
  const [isRefreshing, setIsRefreshing] = useState(false); // isRefreshing state is kept, even if not displayed, as it's managed by loadIncidents

  const loadIncidents = useCallback(async (isAutoRefresh = false) => {
    if (isAutoRefresh) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4"> {/* Modified: Removed justify-between */}
            {currentUser && (
